...
class Dqn():
    ...
    def getBatch(self, model, batchSize):
        # wymiar inputu do sieci (currentState)
        nInputs = self.memory[0][0][0].shape[1]
        # liczba neuronów na wyjściu z ostatniej warstwy modelu sieci
        nOutputs = model.output_shape[-1]
        ...
        randomMemory = randint(0, len(self.memory),
                               size=min(batchSize, len(self.memory)))
        for i in range(len(randomMemory)):
            currentState, action, reward, nextState = \
                self.memory[randomMemory[i]][0]
            gameOver = self.memory[randomMemory[i]][1]
            inputs[i] = currentState
            targets[i] = model.predict(currentState)[0]  # predykcja modelu
            ...
            if gameOver:
                ...
                targets[i][action] = reward + \
                    self.gamma*max(model.predict(nextState)[0])
